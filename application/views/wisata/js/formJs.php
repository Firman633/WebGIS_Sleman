<!-- Make sure you put this AFTER Leaflet's CSS -->
   ...
   <script type="text/javascript">
   	...
   	var marker;
   	var map = L.map('map').setView([-7.7161648, 110.3266482], 12);
	...
	map.on("click",function(e){
		...
		latInput.value=lat;
		lngInput.value=lng;
		//fungsi ajax untuk mengambil nama daerah dari openstreetmap di nominatim
		$.ajax({
		...
		//untuk menentukan nama distrik berdasarkan yang kita klik menggunakan service dari esri(geocodeservices)
	  	geocodeService.reverse().latlng(e.latlng).run(function (error, result) {
	      if (error) {
	        return;
	      }
	      console.log(result);//untuk mengecek apakah service sudah jalan belum
	      var district=result.address.District;
	      //memecah teks inputan
